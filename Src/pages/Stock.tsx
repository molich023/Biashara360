...
import { Plus, Search, Camera, ChevronRight, AlertTriangle, Package, X, Check, Edit3 } from 'lucide-react'
...
export const StockScreen: React.FC = () => {
  ...
  const handlePhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = ev => {
      ...
      setImgPreview(url)
      set('imageUrl', url) // In production: upload to Netlify Blobs, store URL
    }
  ...
  const handleSave = async () => {
    if (!shop || !form.name?.trim()) {
      toast.error('Weka jina la bidhaa.')
      return
    }
    if ((form.sellingPrice || 0) <= 0) {
      toast.error('Weka bei ya uuzaji.')
      return
    }
    setSaving(true)
    try {
    ...
    <div className="page px-4 pt-4">
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="flex gap-2">
            <button onClick={() => setAdjustQty(q => q.startsWith('-') ? q.slice(1) : '-' + q)} className={`px-4 h-12 rounded-xl border font-bold ${adjustQty.startsWith('-') ? 'border-red-500 bg-red-900/30 text-red-400' : 'border-green-500 bg-green-900/30 text-green-400'}`}>
              {adjustQty.startsWith('-') ? '−' : '+'}
            </button>
            <input className="input flex-1 text-xl font-bold text-center" type="number" inputMode="decimal" placeholder="0" value={adjustQty.replace('-','')} onChange={e => set => setAdjustQty((adjustQty.startsWith('-') ? '-' : '') + e.target.value)} />
          </div>
